...
const RequestCallServices = () => {
  ...
  return (
    <div className="flex flex-col md:flex-row items-center justify-between max-w-6xl mx-auto">
      ...
      <h2 className="text-3xl font-bold text-[#091156] mb-2">Request call services</h2>
      ...
      <div className="relative">
          <input
            ...
            className="w-full pl-4 pr-16 py-3 border border-[#8B8B8B] rounded-full focus:outline-none focus:ring-2 focus:ring-[#FF64AE]"
          />
          <button
            type="submit"
            onClick={handleSubmit}
            className="absolute right-0 top-0 bottom-0 bg-[#FF64AE] text-white px-4 rounded-r-full hover:bg-[#FF64AE] focus:outline-none focus:ring-2 focus:ring-[#FF64AE] focus:ring-offset-2"
          >
            <PhoneIcon />
          </button>
        </div>
        <p className="text-sm text-[#8B8B8B] text-end mt-2 ml-4">Toll free for our coverage areas.</p>
      </div>
    </div>
  );
};
